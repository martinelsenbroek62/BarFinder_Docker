...
def decode_audio(audio_language, self_inputindex,
    ...
    try:
        container = client.containers.run(
            ...
            [app.config['ENGINE_CMD'][ENGINE_NAME], 'gpu',
...
def read_transcript(transcript_path, duration, offset, ss_factor):
    ss_factor = int(ss_factor)
    with open(transcript_path) as logfile:
        for line in logfile:
            _, _, start, word_len, word, conf = line.split()
            yield {
                'stime': round(float(start) * ss_factor + offset, 2),
                'duration': round(float(word_len), 2),
                'content': word,
...
@iter_with_timeout()
def xcel2_decode(chunks, audio_language, engine_args):
    ...
    with TemporaryDirectory(dir=shared_dir) as workdir:
        ...
        outpaths = decode_audio(
            audio_language, inputindex, outputindex, ss_factor)
        thread = Thread(
