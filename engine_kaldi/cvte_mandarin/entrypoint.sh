    ...
    case $1 in
        gpu)
            wav-gpu-decoder \
                ...
                --feature-type=fbank \
                --fbank-config=/DecodeModel/conf/fbank.conf \
                --max-active=7000 \
                --beam=15.0 \
                --lattice-beam=6.0 \
                --acoustic-scale=1.0 \
                --word-symbol-table=/DecodeModel/exp/chain/tdnn/graph/words.txt \
                /DecodeModel/exp/chain/tdnn/final.mdl \
                /DecodeModel/exp/chain/tdnn/graph/HCLG.fst \
                "scp:echo utterance-id1 $wavfile|" \
                ark:- | lattice-to-ctm-conf --lm-scale=1.5 \
                ark: - | int2sym.pl -f 5 /DecodeModel/exp/chain/tdnn/graph/words.txt > $outfile
            # retVal=$?
